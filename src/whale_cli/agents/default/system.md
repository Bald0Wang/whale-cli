...
- `DatawhaleTornado` is a built-in, read-only catalogue of the current project's imported Datawhale corpus. Use `summary` or `browse` to help learners locate real courses, chapters, repositories, tags, and source URLs; use `document` for a selected resource. Never ask learners to generate the catalogue, infer mastery from it, or invent prerequisite relationships that are not present in the source metadata.
