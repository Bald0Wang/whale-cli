...
<p align="center">
  ...
  <a href="docs/新手入门/README.md">30 章教程</a> ·
...
| 知识沉淀 | Obsidian Wiki、知识图谱、知识龙卷、间隔复习、学习档案、学习项目空间 | 学过什么、知识如何关联、缺什么前置、何时复习，以及每个学习方向的数据边界 |

## 五分钟开始

### 1. 安装

在本 README 所在目录执行。CLI 需要 **Python 3.10+**；使用 WebUI 还需要 **Node.js 18+** 和 npm。
...
如果 PowerShell 提示脚本执行被禁止，请仅对当前用户执行一次 `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned`，重新打开 PowerShell 后再激活虚拟环境。

### 2. 配置模型

Whale 默认使用 OpenAI 兼容的 Step Plan 接口和 `step-3.7-flash`。

#### `step-3.7-flash`：macOS / Linux

```bash
export STEP_API_KEY="your-step-plan-key"
```

#### `step-3.7-flash`：Windows PowerShell
...
#### `step-explore`：Windows PowerShell

```powershell
$env:STEPFUN_API_KEY="your-step-explore-key"
$env:LLM_MODEL="step-explore"
...
```powershell
New-Item -ItemType Directory -Force -Path "$HOME\.whale" | Out-Null
Copy-Item config.example.json "$HOME\.whale\config.json"
```
...
#### macOS / Linux 与 Windows PowerShell
...
| 21-29 学习陪伴 | 想把项目用在真实学习上 | 拆解目标、生成路线、确认完成、复习并沉淀成果 | 学习项目、知识龙卷、图谱、路线 checklist、复习表、档案 |
...
| [29 知识龙卷：把 Datawhale 内容做成学习产品](docs/新手入门/29-知识龙卷-把Datawhale内容做成学习产品.md) | 进阶 | 45 分钟 | 自动整理当前项目的完整 Datawhale 语料，按领域、课程和来源浏览真实资料。 |

**WebUI 配合**：先创建一个学习项目，在 **知识龙卷** 浏览当前项目已导入的 Datawhale 课程、章节和项目资料；再让对话基于真实资料规划路线，并在 **学习路线** 勾选 checklist。然后打开 **学习图谱** 查看长期概念关系，在 **间隔复习** 中完成复习，最后到 **学习档案** 浏览可展示的证据。路线和完成状态应由学习者确认，而不是由模型自行宣告。
...
```powershell
New-Item -ItemType Directory -Force -Path ".whale_cli" | Out-Null
Copy-Item mcp.example.json ".whale_cli\mcp.json"
whale-cli
```
