...
Whale CLI 会在当前目录下工作——它读取、搜索和修改的都是当前目录里的文件。后续章节的运行方式相同，只是换成 `chapter-02`、`chapter-03`……到第 8 章，代码积累到一定程度后会打包成 `whale` 命令，届时直接在项目目录里输入 `whale` 就能进入交互式界面。
...
<figure class="chapter-figure">
  ...
  <figcaption><strong>第 1 章 · 模型发请求，本地程序打开文件</strong>：`tool_call` 只描述动作；磁盘访问发生在 Whale CLI 的本地进程里。</figcaption>
</figure>
...
<figure class="chapter-figure">
  ...
  <figcaption><strong>第 1 章 · 一次工具调用，messages 多出两条</strong>：`assistant.tool_calls` 记录请求，`tool` 记录与 `call_7` 对应的结果。</figcaption>
</figure>
...
以下是使用 DeepSeek v4-flash 的真实运行记录。模型知道要找什么，但不知道代码在哪个文件里。它只能根据训练中见过的项目惯例猜路径：

```text
⏺ read_file(.)                           ← 路径是目录，读取失败
⏺ read_file(README.md)                   ← 读了，没有函数定义
⏺ read_file(package.json)                ← 读了，没有函数定义
⏺ read_file(src/tools)                   ← 路径是目录
⏺ read_file(src/tools/agent-tools.ts)    ← 不存在
⏺ read_file(src/agents)                  ← 路径是目录
...
Error: 达到最大轮数 10，任务仍未完成
```

模型猜了 `src/tools/agent-tools.ts`——看起来合理，但这个文件不存在。它还尝试直接读目录路径，也失败了。10 轮全部用完，函数没找到。

实际上 `evaluateToolAvailability` 就在 `src/tools/availability.ts` 里。模型差一步——它找到了 `src/tools` 目录，但没有办法看到目录里有哪些文件。
